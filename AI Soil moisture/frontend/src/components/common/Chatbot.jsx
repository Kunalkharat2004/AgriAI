  ...
  return (
    <>
      ...
      >
        ...
        <div className="bg-gradient-to-r from-emerald-500 to-green-600 text-white px-6 py-4 rounded-t-2xl flex justify-between items-center">
          <div className="flex items-center flex-1">
            ...
            <div className="relative" ref={languageDropdownRef}>
              <button
                onClick={() => setShowLanguageDropdown(!showLanguageDropdown)}
                className="bg-white bg-opacity-20 hover:bg-opacity-30 px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50 backdrop-blur-sm"
              >
              ...
              {showLanguageDropdown && (
                <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl border border-gray-200 py-1 min-w-[140px] z-50 overflow-hidden backdrop-blur-sm">
                  {Object.entries(LANGUAGES).map(([code, lang]) => (
                    <button
                      key={code}
                      onClick={() => handleLanguageChange(code)}
                      className={`w-full text-left px-4 py-3 hover:bg-emerald-50 transition-colors text-gray-700 font-medium ${
                        selectedLanguage === code ? 'bg-emerald-50 text-emerald-700 border-r-4 border-emerald-500' : ''
                      }`}
                    >
                      {lang.name}
                    </button>
        ...
        {/* Chat messages */}
        <div className="flex-1 p-4 overflow-y-auto custom-scrollbar bg-gray-50" style={{ height: 'calc(100% - 128px)' }}>
          {messages.map((msg, index) => (
            <div
              key={msg.id}
              className={`flex items-start mb-4 ${
        ...
        <div className="p-4 bg-white border-t border-gray-100 rounded-b-2xl">
          <div className="flex items-center gap-3 bg-gray-50 rounded-2xl p-3 border border-gray-200">
            ...
            <input
              type="text"
              ref={inputRef}
              className="flex-1 bg-white border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-gray-800 text-sm placeholder-gray-500"
              placeholder={currentLang.placeholderText}
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              onKeyPress={handleKeyPress}
